<?php
	...
	class Data_Handler extends Arr_Handler{
		...
		function getResultOfAnalyse($arr){
		    ...
		    $var1['collateral_value'] = self::getCollateral_value($var1);
			return $var1;
		}
		
		function getCollateral_value($date){
			/*传入均价，根据这个值求出评估值*/
			$max1 = 63000;
			$max2 = 50000;		//原来是46000
			// $min1 = 8000;		
			...
			$min1 = 15000;
			$min2 = 12750;
			$ratio = $min2/$min1 - ($min2/$min1-$max2/$max1)/($max1-$min1)*($date['average']-$min1);
			$new_value = round($date['average']*$ratio,0);
			
			// 接下来与最小挂牌价比较一下，但这个最小挂牌价要剔除异常值
			// $low = ($date['v25']-$date['v75'])*1.5 + $date['v25'];
			$low = max(($date['v25']-$date['v75'])*1.5 + $date['v25'],$date['min'] );
